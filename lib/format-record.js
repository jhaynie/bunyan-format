...
module.exports = function formatRecord(rec, opts) {
  ...
  case OM_LONG:
	...
	if (opts.timestampInString === undefined || opts.timestampInString) {
		fmtString += '%s ';
		fmtArgs.push(time);
	}
	if (opts.levelInString === undefined || opts.levelInString) {
		fmtString += '%s ';
		fmtArgs.push(level);
	}
	if (opts.nameInString === undefined || opts.nameInString) {
		fmtString = fmtString.trim() + ': %s';
		fmtArgs.push(nameStr);
	}
	if (!short && (opts.hostnameInString === undefined || opts.hostnameInString)) {
		fmtString += 'on %s ';
		fmtArgs.push(hostname || '<no-hostname>');
	}
	if (!short && (opts.sourceInString === undefined || opts.sourceInString)) {
		fmtString = fmtString.trim() + '%s';
		fmtArgs.push(src);
	}
	fmtString +='%s %s\n';
	fmtArgs.push(onelineMsg);
	fmtArgs.push(extras);
	fmtArgs.push(details);
	return format.apply(null, [fmtString].concat(fmtArgs));
